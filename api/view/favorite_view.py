...
class FavoriteViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request: Request, *args, **kwargs):
        ...
        user = CustomeUser.objects.filter(eoa=eoa)
        lecture = Lecture.objects.get(id=lectureId)
        # print(Favorite.objects.filter(user=user, lecture=lecture))
        favo = Favorite.objects.filter(user=user, lecture=lecture)
        print(favo.vote_weight)

        if not (Favorite.objects.filter(user=user, lecture=lecture) is None):
